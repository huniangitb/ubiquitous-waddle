...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupUI() {
        ...
        audioAdapter = AudioListAdapter { audioItem ->
            mediaService?.playSongAtIndex(audioList.indexOf(audioItem))
        }
    ...
    private fun setupSliderListeners() {
        binding.playbackSlider.addOnSliderTouchStopListener { slider, _ ->
        ...
        // Save preferences only when user finishes interaction
        val touchStopListener = Slider.OnSliderTouchStopListener { slider, _ ->
            when (slider.id) {
        ...
        binding.earpieceGainSlider.addOnSliderTouchStopListener(touchStopListener)
        binding.speakerGainSlider.addOnSliderTouchStopListener(touchStopListener)
        binding.delaySlider.addOnSliderTouchStopListener(touchStopListener)
        binding.highPassFilterSlider.addOnSliderTouchStopListener(touchStopListener)
        binding.lowPassFilterSlider.addOnSliderTouchStopListener(touchStopListener)
    }
    ...
    private fun loadSettings() {
        ...
        // Trigger change listeners to apply loaded settings
        binding.earpieceGainSlider.dispatchTouchEvent(android.view.MotionEvent.obtain(0,0,android.view.MotionEvent.ACTION_UP,0f,0f,0))
    }
    ...
    private fun updateUI(isPlaying: Boolean, currentIndex: Int, currentPosition: Int, duration: Int) {
        ...
        if (currentIndex in audioList.indices) {
            binding.statusTextView.text = audioList[currentIndex].title
        }
    ...
    override fun onStart() {
        super.onStart()
        registerReceiver(updateReceiver, IntentFilter(MediaPlaybackService.ACTION_UPDATE_UI))
    }
